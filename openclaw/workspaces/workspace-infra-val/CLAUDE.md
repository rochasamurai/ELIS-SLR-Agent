# CLAUDE.md — workspace-infra-val (Claude Code Workflow Rules)
> Auto-loaded into every Claude Code session for this workspace.
> Survives context compression. Treat every rule as always active.
...
2. Read `CURRENT_PE.md` at repo root:
   - Confirm `infra-val-claude` appears in the `Agent roles` table.
...
4. Wait for explicit PM assignment before beginning review.
   Signal: PM PR comment or CI Gate 1 comment assigning `infra-val-claude`.
