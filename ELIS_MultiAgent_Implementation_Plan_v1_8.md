...
> **Supersedes:** `ELIS_MultiAgent_Implementation_Plan_v1_7.md`
...
Architecture v1.8 makes a narrower and more operationally realistic decision than v1.7: `elis-server` does not need to run every SLR workload locally. Instead, workload placement should follow task shape, hardware fit, and governance needs.

This plan therefore implements a hybrid SLR execution model:
...
The objective is not maximum localism. The objective is:

**the cheapest safe execution surface that preserves quality, reproducibility, auditability, and operator control.**

### What v1.8 changes

v1.8 adds an SLR-specific execution placement layer on top of the existing governed ELIS platform:

- formal local Screening capability on `elis-server`
- formal local lightweight support-agent capability on `elis-server`
- explicit retention of Harvest on GitHub workflows
- explicit non-migration of Extraction and Synthesis for the current host generation
- capacity-aware throttling and placement rules
...
All of the following must be true before beginning the v1.8 series:
...
| 2 | PE-SLR-03: ASReview screening pilot | 2 | `infra-impl-claude` | — |
| 2 | PE-SLR-04: Local screening governance and evidence | 2 | `infra-impl-codex` | — |
| 3 | PE-SLR-05: Metadata triage and query refinement | 3 | `prog-impl-claude` | — |
| 3 | PE-SLR-06: Bibliometric clustering and discrepancy pre-analysis | 3 | `prog-impl-codex` | — |
| 4 | PE-SLR-07: Extraction off-host contract | 4 | `infra-impl-claude` | — |
| 4 | PE-SLR-08: Synthesis off-host contract | 4 | `infra-impl-codex` | — |
| 5 | PE-SLR-09: `elis-server` capacity and placement policy enforcement | 4 | `infra-impl-claude` | — |
| 5 | PE-SLR-10: End-to-end hybrid SLR validation | 4 | `infra-impl-codex` | — |
...
The v1.8 series mixes SLR and infrastructure-governance work.
...
All v1.8 PEs target `main` unless PM explicitly defines a different release line in `CURRENT_PE.md`.
...
| R-08 | Architecture drift between target-state v1.8 and live deployment | Medium | end-to-end hybrid validation in PE-SLR-10 |
...
2. Screening runs successfully as a governed local-first capability on `elis-server`.
3. At least one lightweight local support-agent tier is operational on `elis-server`.
4. Extraction remains off-host with an explicit workflow contract.
5. Synthesis remains off-host with an explicit workflow contract.
6. PM can report execution surface by SLR phase accurately.
7. `elis-server` capacity and throttling policy is committed and operator-usable.
8. One representative hybrid SLR flow is validated end to end.
9. No architectural invariant requires `elis-server` to run all SLR agents concurrently.
10. The resulting system preserves governance, reproducibility, and auditability across all execution surfaces.

---

*ELIS SLR Agent — Multi-Agent Implementation Plan · v1.8 · April 2026 · Hybrid SLR Execution · Host: ELIS MiniServer NUC8i7BEH (`elis-server`)*
