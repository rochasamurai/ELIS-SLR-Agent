...
Notes
-----
- Accepts A as either a top-level list or {"rows": [...] } (back-compat).
- Writes *only* B_FILE and C_FILE (MVP path).
- Tests may override ART_DIR / A_FILE / B_FILE / C_FILE.
"""
...
# --------------------------------------------------------------------------- #
# Locations (tests override these module variables)
# --------------------------------------------------------------------------- #
...
def run() -> Dict[str, Any]:
    ...
    Returns dict with counters 'a', 'b', 'c' and paths written.
    """
    ...
    # Return shape expected by tests (a/b/c counters) plus paths
    return {
        "a": len(a_rows),
        "b": len(b_rows),
        "c": len(c_rows),
