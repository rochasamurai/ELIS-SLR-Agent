...
Purpose
-------
Produce minimal, schema-aligned artefacts for Appendices A, B, and C so CI can smoke-test the
pipeline. If an Appendix A JSON file exists (from a prior step), use its content as input; otherwise
generate a dummy search query entry. Then produce screening decisions (Step B) with a placeholder
reason for each decision, and a minimal extraction record for included items. Files are written
...
def run():
    """
    Execute the toy agent logic: read Appendix A input (if available), generate screening decisions
    (Appendix B) and extraction records (Appendix C), and write them to disk.

    Returns
    -------
    dict: with keys 'a', 'b', 'c' corresponding to the lists of records for Appendices A, B, and C.
    """
    # Appendix A — Search (use existing input if available, else create a dummy record)
    a_rows = []
    if A_FILE.exists():
        try:
    ...
    # Appendix B — Screening (create one included and one excluded decision)
    # Link screening entries to the first search record via source_id.
    ...
    # Generate an extraction entry for the included screening record.
    c_rows = []
    if b_rows:
        included_entry = next(
