...
from semanticscholar_harvest import (
    ...
    transform_semanticscholar_entry,
)
...
class BenchmarkSearcher:
    ...
    def search_all_databases(self) -> List[Dict]:
        ...
        try:
            ...
            results = semanticscholar_search(
                simple_query, limit=100, max_results=100
            )  # Reduced from 200
            normalized = [
                self._normalize_entry(
                    transform_semanticscholar_entry(r), "semanticscholar"
                )
                for r in results
            ]
        ...
        try:
            ...
            normalized = [
                self._normalize_entry(transform_openalex_entry(r), "openalex")
                for r in results
            ]
            print(f"  Found {len(normalized)} results")
            all_results.extend(normalized)
        ...
        try:
            ...
            normalized = [
                self._normalize_entry(transform_core_entry(r), "core") for r in results
            ]
            print(f"  Found {len(normalized)} results")
            all_results.extend(normalized)
        except Exception as e:
        ...
        try:
            ...
            normalized = [
                self._normalize_entry(transform_crossref_entry(r), "crossref")
                for r in results
            ]
            print(f"  Found {len(normalized)} results")
            all_results.extend(normalized)
        ...
        try:
            ...
            normalized = [
                self._normalize_entry(transform_scopus_entry(r), "scopus")
                for r in results
            ]
            print(f"  Found {len(normalized)} results")
            all_results.extend(normalized)
        ...
        try:
            ...
            normalized = [
                self._normalize_entry(transform_wos_entry(r), "wos") for r in results
            ]
            print(f"  Found {len(normalized)} results")
            all_results.extend(normalized)
        except Exception as e:
        ...
        try:
            ...
            normalized = [
                self._normalize_entry(transform_ieee_entry(r), "ieee") for r in results
            ]
            print(f"  Found {len(normalized)} results")
            all_results.extend(normalized)
        except Exception as e:
        ...
        try:
            ...
            normalized = [
                self._normalize_entry(
                    transform_google_scholar_entry(r), "google_scholar"
                )
                for r in results
            ]
            print(f"  Found {len(normalized)} results")
