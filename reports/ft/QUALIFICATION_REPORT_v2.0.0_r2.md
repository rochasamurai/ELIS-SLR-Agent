# ELIS v2.0.0 Functional Qualification Report (r2)

Candidate SHA: d071bdd3290e2fa31cb545487416925664c1ba9a
Date: 2026-02-18
Executor: CODEX
...
| Suite | Result | Notes |
|---|---|---|
| FT-01 | FAIL | Packaging fix validated for `elis validate` import path, but `elis merge --from-manifest DOES_NOT_EXIST.json` still raises unhandled traceback (`FileNotFoundError`) instead of controlled CLI error. |
| FT-02 | NOT RUN | Pre-FT-02 secrets gate failed (`SCOPUS_API_KEY`, `ELIS_CONTACT` missing). |
| FT-03 | NOT RUN | Stopped after FT-01 failure per ordered execution rule. |
| FT-04 | NOT RUN | Stopped after FT-01 failure per ordered execution rule. |
| FT-05 | NOT RUN | Stopped after FT-01 failure per ordered execution rule. |
| FT-06 | NOT RUN | Stopped after FT-01 failure per ordered execution rule. |
| FT-07 | NOT RUN | Stopped after FT-01 failure per ordered execution rule. |
| FT-08 | NOT RUN | Stopped after FT-01 failure per ordered execution rule. |
| FT-09 | NOT RUN | Stopped after FT-01 failure per ordered execution rule. |
| FT-10 | NOT RUN | Stopped after FT-01 failure per ordered execution rule. |
| FT-11 | NOT RUN | Stopped after FT-01 failure per ordered execution rule. |
| FT-12 | NOT RUN | Stopped after FT-01 failure per ordered execution rule. |

## Blocking Findings
1. FT-01 contract failure: `elis merge --from-manifest DOES_NOT_EXIST.json` exits non-zero but with raw traceback (`FileNotFoundError`) rather than controlled CLI error output.
2. Pre-FT-02 gate failure: required environment secrets missing (`SCOPUS_API_KEY`, `ELIS_CONTACT`).

## Non-Blocking Findings
1. Packaging hotfix validation succeeded for prior FT-01 failure: `elis validate DOES_NOT_EXIST.json` no longer raises `ModuleNotFoundError`; now returns controlled CLI message.
...
```text
$ git rev-parse HEAD
d071bdd3290e2fa31cb545487416925664c1ba9a
```

### Environment Setup (r2)
Artifact: `reports/ft/r2/setup/00_environment_setup.log.txt`
...
Building wheels for collected packages: elis-slr-agent
  ...
  Created wheel for elis-slr-agent: filename=elis_slr_agent-2.0.0-0.editable-py3-none-any.whl size=3144 sha256=e0c9910a5380b07deff092c20bab573fb3443da27e4f65459dd6670501554216
  Stored in directory: C:\Users\carlo\AppData\Local\Temp\pip-ephem-wheel-cache-hqggorxz\wheels\ee\93\c5\aeabeb4ac85bbca275337b450409549be93d92b94d71efcd82
Successfully built elis-slr-agent
Installing collected packages: elis-slr-agent
  Attempting uninstall: elis-slr-agent
    Found existing installation: elis-slr-agent 2.0.0
    Uninstalling elis-slr-agent-2.0.0:
      Successfully uninstalled elis-slr-agent-2.0.0
Successfully installed elis-slr-agent-2.0.0


```

### FT-01: CLI Contract (r2)
Artefacts: `reports/ft/r2/FT-01/01.log.txt` ... `reports/ft/r2/FT-01/11.log.txt`
...
ExitCode: 1
Output:
Provide both <schema_path> and <json_path>, or neither.
...
### Gate Check Before FT-02 (r2)
Artifact: `reports/ft/r2/gate/pre_ft02_secrets_check.log.txt`
...
### FT-02 through FT-12
Not executed due blocking findings above.
