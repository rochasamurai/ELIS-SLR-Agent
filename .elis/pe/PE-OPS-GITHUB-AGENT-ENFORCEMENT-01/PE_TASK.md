...
## Background
PR #429 showed a wrong-path GitHub operation. PR #430 was a one-time fallback exception. This PE documents and enforces the source-path rules so GitHub Agent always resolves exactly one authorised PR source path and fails closed otherwise.
