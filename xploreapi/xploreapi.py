...
class XPLORE:
    ...
    bioEndPoint = "http://ieeexploreapi.ieee.org/api/v1/author/"
    ...
    imageFields = {
        ...
        "authors": "metadata[i].authors.authors[j]",
        "author_full_name": "metadata[i].authors.authors[j].full_name",
        "author_affiliation": "metadata[i].authors.authors[j].affiliation",
        "author_profile_url": "metadata[i].authors.authors[j].authorUrl",
        "author_order": "metadata[i].authors.authors[j].author_order",
        "content_type": "metadata[i].content_type",
        "abstract": "metadata[i].abstract",
        "article_number": "metadata[i].article_number",
  ...
  # Fetch cltoken (for Images & Fulltext)
    def getImageToken(self, auth_token: str) -> str:
        ...
        data = urllib.parse.urlencode({"auth-token": auth_token})

        response = requests.post(
            f"{self.authTokenEndPoint}?apikey={self.apiKey}", headers=headers, data=data
        )
    ...
    def searchImages(
        ...
        """
        Search IEEE Images with filters.
        Requires cltoken.
        """
        if not self.cltoken:
            raise Exception("cltoken missing. Call getImageToken(auth_token) first.")
        ...
        params = {
            ...
            "format": format,
            "apikey": self.apiKey,
            "cltoken": self.cltoken,
        }
        if image_keyword:
            params["image_keyword"] = image_keyword
        if content_type:
        ...
        # 🔥 Parse new Image API fields
        if "images" in data:
            print("\n📸 Image URLs and metadata returned:")
    ...
    # Download images from a specific article
    def getArticleImages(self, article_number: str, save_folder="images") -> list:
        """
        Download all images from a given IEEE article.
        Requires cltoken.
        """
        if not self.cltoken:
            raise Exception("cltoken missing. Call getImageToken(auth_token) first.")
        ...
        url = f"{self.imageEndPoint}{article_number}?apikey={self.apiKey}&format=json&cltoken={self.cltoken}"
        ...
        print(f"\n📑 Article {article_number} Image URLs and metadata:")
        for i, img in enumerate(data["images"], start=1):
            img_url = img.get("figure", {}).get("graphic", {}).get("large")
            img_type = img.get("imageType", "N/A")
            figure_id = img.get("figure", {}).get("id", "N/A")
            figure_label = img.get("figure", {}).get("label", "N/A")
            figure_caption = img.get("figure", {}).get("caption", "N/A")
