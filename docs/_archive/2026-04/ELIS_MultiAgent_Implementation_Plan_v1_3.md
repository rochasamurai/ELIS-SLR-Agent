...
## Implementation Plan · Version 1.3 · March 2026
...
| v1.3 | Mar 2026 | Aligned to Architecture v1.5: VPS/Hostinger replaced by MiniServer (NUC8i7BEH elis-server) throughout; agent roster expanded from 13 to 19 agents with SLR domain phase-specialised (Harvest, Screen, Extraction, Synthesis, PRISMA); openclaw.json and volume layout updated accordingly; PE-VPS-00 rescoped to MiniServer baseline |
...
|---|---|
| 2 agents (CODEX + Claude Code) | 19 agents (PM + 18 workers) |
...
| `harvest-impl-codex` | Harvest Implementer | CODEX | — | SLR — Harvest |
| `harvest-val-claude` | Harvest Validator | Claude Code | CODEX output | SLR — Harvest |
| `screen-impl-claude` | Screen Implementer | Claude Code | — | SLR — Screen |
| `screen-val-codex` | Screen Validator | CODEX | Claude Code output | SLR — Screen |
| `extract-impl-codex` | Extraction Implementer | CODEX | — | SLR — Extraction |
| `extract-val-claude` | Extraction Validator | Claude Code | CODEX output | SLR — Extraction |
| `synth-impl-claude` | Synthesis Implementer | Claude Code | — | SLR — Synthesis |
| `synth-val-codex` | Synthesis Validator | CODEX | Claude Code output | SLR — Synthesis |
| `prisma-impl-claude` | PRISMA Implementer | Claude Code | — | SLR — PRISMA |
| `prisma-val-codex` | PRISMA Validator | CODEX | Claude Code output | SLR — PRISMA |

> **Note:** `infra-val-codex` and `infra-val-claude` were registered via PE-OC-19; workspace created by PE-OC-21 (merged 2026-02-26). The SLR domain has been expanded from 4 generic agents to 10 phase-specialised agents per Architecture v1.5 §4.2. All 19 agents are defined; SLR phase agents are provisioned in the PE-ELIS series.
...
The PM Agent is the only agent bound to the Discord channel. All 18 worker agents are internal — unreachable from outside OpenClaw. The PO interacts exclusively with the PM Agent in natural language via Discord.
...
All sensitive values in `~/.openclaw` — including API keys, the Discord bot token, and session state — are stored via Docker secrets, not as plaintext config files. See Architecture v1.5 §8.
...
| `~/openclaw/workspace-slr-harvest` | Harvest agent pair AGENTS.md, CLAUDE.md, CODEX.md | Container: **read-only** |
| `~/openclaw/workspace-slr-screen` | Screen agent pair AGENTS.md, CLAUDE.md, CODEX.md | Container: **read-only** |
| `~/openclaw/workspace-slr-extract` | Extraction agent pair AGENTS.md, CLAUDE.md, CODEX.md | Container: **read-only** |
| `~/openclaw/workspace-slr-synth` | Synthesis agent pair AGENTS.md, CLAUDE.md, CODEX.md | Container: **read-only** |
| `~/openclaw/workspace-slr-prisma` | PRISMA agent pair AGENTS.md, CLAUDE.md, CODEX.md | Container: **read-only** |
...
```json
{
  "agents": {
    "list": [
      ...
      { "id": "harvest-impl-codex",  "workspace": "~/.openclaw/workspace-slr-harvest" },
      { "id": "harvest-val-claude",  "workspace": "~/.openclaw/workspace-slr-harvest" },
      { "id": "screen-impl-claude",  "workspace": "~/.openclaw/workspace-slr-screen" },
      { "id": "screen-val-codex",    "workspace": "~/.openclaw/workspace-slr-screen" },
      { "id": "extract-impl-codex",  "workspace": "~/.openclaw/workspace-slr-extract" },
      { "id": "extract-val-claude",  "workspace": "~/.openclaw/workspace-slr-extract" },
      { "id": "synth-impl-claude",   "workspace": "~/.openclaw/workspace-slr-synth" },
      { "id": "synth-val-codex",     "workspace": "~/.openclaw/workspace-slr-synth" },
      { "id": "prisma-impl-claude",  "workspace": "~/.openclaw/workspace-slr-prisma" },
      { "id": "prisma-val-codex",    "workspace": "~/.openclaw/workspace-slr-prisma" }
    ]
  },
  "bindings": [
    {
...
All 24 PEs in this release line follow the 2-agent governance model. For the OC/INFRA execution record, CODEX implements odd-numbered PEs and Claude Code implements even-numbered PEs. The MiniServer corrective baseline PE (PE-VPS-00) is tracked as a prerequisite infrastructure PE with explicit Implementer/Validator assignment. Validation is always performed by the opposite engine. Each PE follows the full governance cycle: `HANDOFF.md` → PR → Gate 1 (Validator assignment) → `REVIEW_<PE>.md` → Gate 2 (merge or iteration).

**Manifest compliance note (Architecture v1.5 §3.1, effective March 2026):** Any PE that generates or processes ELIS run artifacts must confirm that `run_manifest.json` is generated and validated against `run_manifest.schema.json` before PASS is issued. This requirement applies prospectively from Architecture v1.5 adoption. PE-OC-21 is the first PE subject to this requirement at time of writing.
...
> **Goal:** Create the eleven workspace variants for the 18 worker agents. Each workspace contains only the rules relevant to its role and domain. Worker agent sessions are launched manually by the PM until Gate automation (Phase 3) is complete.
...
### Phase 7 — MiniServer Corrective Baseline

> **Goal:** Restore the missing MiniServer execution prerequisite by introducing a dedicated host baseline provisioning PE before any further MiniServer functional work.

---

#### PE-VPS-00 · MiniServer Baseline Provisioning (Corrective)
...
- Validate and document the ELIS MiniServer (NUC8i7BEH · Ubuntu 24.04.4 LTS · `elis-server`) baseline configuration
- Confirm SSH key-only auth, disable password auth, configure UFW, enable fail2ban
- Confirm Docker 28.2.2 and Docker Compose 2.37.1 operational (`docker info`, `docker compose version`)
- Confirm `/opt/elis/` directory structure and ELIS repo clone at `/opt/elis/repo`
- Confirm ELIS CLI v2.0 installed and functional (`elis --help`)
- Confirm OpenClaw gateway is not internet-exposed (localhost-only access)
- Produce baseline runbook and validation evidence prior to any new MiniServer feature PE
...
5. Baseline verification artifact committed and reviewed (`docs/_active/MINISERVER_BASELINE.md`, `REVIEW_PE_VPS_00.md`)
...
| 18 | PE-VPS-00: MiniServer Baseline Provisioning (Corrective) | Phase 7 | CODEX | 2–4h | OC-21 | Planned |
| **Total** | **24 PEs** | **6 Phases + governance + MiniServer baseline** | **CODEX×14 · Claude Code×10** | **69–95h** | **~18 wks planned** | |
...
Architecture v1.5 §3.1 (Invariant 6) requires that all run artifacts are reproducible from a validated `run_manifest.json`. This requirement applies to all PEs generating or processing ELIS run artifacts, effective from Architecture v1.5 adoption (March 2026). PE-OC-21 is the first PE subject to this requirement. Validators must confirm manifest compliance as part of acceptance criteria for any PE producing run artifacts.
...
| R-07 | Run artifact produced without valid manifest (silent manifest bypass) | Low (if CI enforced) | Architecture v1.5 §3.1 Invariant 6. CI manifest gate enforced from PE-OC-21 onwards. |
| R-08 | MiniServer not baselined/hardened before MiniServer execution starts | Medium | PE-VPS-00 is a blocking prerequisite for subsequent MiniServer work; no MiniServer PE starts before PE-VPS-00 PASS. |
...
10. PE-VPS-00 (MiniServer baseline provisioning) merged with PASS verdict before any future MiniServer functional PE begins
...
*ELIS SLR Agent — Multi-Agent Implementation Plan · v1.3 · March 2026 · Aligned to Architecture v1.5 · Host: ELIS MiniServer NUC8i7BEH (elis-server)*
