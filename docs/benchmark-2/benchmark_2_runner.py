...
class FuzzyMatcher:
    ...
    @staticmethod
    def match_study(
        gold_study: Dict, retrieved_study: Dict, threshold: float = 0.85
    ) -> bool:
        """
        ...
        if title_sim >= threshold:
            ...
            retrieved_authors = FuzzyMatcher.normalize_text(
                retrieved_study.get("authors", "")
            )
            ...
            if gold_authors and retrieved_authors:
                ...
                retrieved_surnames = [
                    w for w in retrieved_authors.split() if len(w) > 2
                ]
...
class Benchmark2Executor:
    ...
    def execute_phase1(self) -> BenchmarkResult:
        ...
        matched_studies, missed_studies = self.match_studies(
            self.gold_standard, unique_retrieved
        )
        ...
        print(
            f"Matched: {total_matched}/{len(self.gold_standard)} ({retrieval_rate:.1%})"
        )
